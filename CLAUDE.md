...
## 🧪 本番へcurl等で動作確認するときは「テストだと分かる形」にする（2026-09-16再発防止）

DB分割の本番切替(2026-09-14〜15)直後、動作確認のため`curl`で
`POST /api/auth/signup`を直接叩いて新規ユーザーを1件作成し、確認後に
削除した（誰が行ったか確証は無いが、タイミング的にDB分割作業を担当した
Claude Codeセッションによる自動確認だった可能性が高い）。この後片付け
自体は正しい判断だったが、**「テストで作って消した」という記録がどこにも
残らなかった**ため、後日「登録完了ファネルの数字と実際のユーザー一覧が
食い違う＝本番データが消失したのでは」という重大インシデント調査を
まる1セッション分行う羽目になった（結果的にはデータ消失ではなく誤検知
だったが、本物の事故と見分けがつかず、本番DBへの読み取り専用調査で
すら気を遣う一因になった）。

- 本番環境に対してcurl等でAPIを直接叩いて動作確認する際、**実データと
  混同されないよう「テストだと分かる」形にする**:
  - 可能なら確認後すぐに削除するのではなく、`is_test=1`を立てておく
    （`app/routers/system.py`の`admin_set_test_flag`エンドポイント等）。
    削除してしまうと、AUTOINCREMENTカウンタ(`sqlite_sequence`)だけが
    進んで実データとは一致しない状態になり、後から見ると「行が消えた」
    ようにしか見えない。
  - メールアドレスに`claude-verify-`等はっきり分かる接頭辞を使う。
  - 確認・削除を行ったら、その場で`docs/TODO.md`（このセッションの
    作業ログ）に一言残す（「9/15 13:38 JST DB分割後の動作確認でcurl
    signup 1件作成→削除」等）。次回別セッションが不審な差分に気づいた
    ときに即座に説明がつく。
- 副次的に見つかった不具合として、管理画面「登録に至らない原因分析」
  (`admin_registration_funnel`・`app/routers/system.py`)のボット除外は
  「訪問(visit)記録のUA」でしか判定しておらず、visitを経由せず
  signupへ直行するcurl等はすり抜けていた。`kind='signup'`行自体のUAも
  判定対象に含めるよう修正済み。

## 🏷️ Googleタグ(広告)まわりの必須ルール(2026-09-21)

サイトの主要ページ(`static/`のindex/login/about/privacy/terms)にはGoogle広告のタグ(gtag)が入っている。
広告の効果測定のためだけに使う方針(オーナー判断: リマーケティングは使わない)で、次を守る。

- **ローカルのブラウザ確認でタグ付きページを開くと、実際にオーナーのGoogle広告アカウントへ通信が飛ぶ**
  (127.0.0.1が「新しいドメイン」として検知され、広告管理画面に「クロスドメイン測定に含めるドメインを
  追加」が出た)。Playwright等でタグ付きページを動かすときは、`googletagmanager.com`のgtag.js取得だけ通し、
  google/doubleclick/googleadservices/gstatic宛ての通信は`route.abort()`で遮断すること
  (実装例は`memory/reference-local-e2e-testing`)。本番サイトに対する動作確認でも同様に、通信を遮断するか、
  タグの無いページ(`/api/...`等)で行う。
- **タグに入れている`gtag('set','allow_ad_personalization_signals', false)`を勝手に外さない**
  (プライバシーポリシー§7の利用目的「広告の効果測定」との一致のため)。外すのはオーナーがリマーケティングを
  使うと決めたときだけで、その場合は§7に利用目的を追記する。
- **決済・ログイン後の画面(`/paypay-charge`等)にタグを入れない**。PayPayの戻りURLに決済の識別番号
  (`?mpid=`)が付き、ページURLごとGoogleに送られるため。Google広告の「タグの品質: 要確認」に
  `/paypay-charge`が出るのは想定内で、無視してよい。
- タグの有無を変える、または新しい外部送信(アナリティクス連携・SNSのピクセル等)を足すときは、
  プライバシーポリシー§7と§9を同時に直す(記載に無い外部送信を作らない)。
