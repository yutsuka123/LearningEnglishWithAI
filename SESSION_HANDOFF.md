...
### 残っている作業（最優先）
**例文訳の人手点検が 6,729語 残っている**（誤りはID 9400以降に強く偏る）。
手順・注意点・進捗の見かたは、private リポジトリ `business_plan` の
`products/study_nyangailab/訳文点検_引き継ぎ.md` にまとめてある。
（`docs/` はgit管理外で他マシンに来ないため、そちらに置いた。）

進捗は `detail.example_ja_checked` としてDBに入っているので、
`scripts/list_example_ja_todo.py` を流せば残りだけが出てくる。
