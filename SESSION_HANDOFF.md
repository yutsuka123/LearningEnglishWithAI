...
> 本リポジトリは公開（public）。個人情報・インフラ接続情報・事業戦略の詳細は書かない
> 方針（2026-08-03〜）。ビジネス戦略・収益化検討は別リポジトリ `business_plan`
> （private）を参照。
...
## 本番環境（サクラVPS）
- **接続情報・パス等の詳細は非公開ドキュメントへ移設**（`docs/TODO.md`「2026-08-03
  （続き3）」参照、gitignore対象）。本ファイルは公開リポジトリのため、インフラの
  接続情報は載せない方針に変更（2026-08-03）。
...
- **個別テスターの利用状況（非公開ドキュメントへ移設）**: `docs/TODO.md`「2026-08-03
  （続き3）」参照。本ファイルは公開リポジトリのため、個人が特定できる利用データは
  載せない方針に変更（2026-08-03）。
- コスト超過・エラーの兆候なし（差し支えない範囲の要約）。

## 2026-08-22 の引き継ぎ（ver1.2.10・作業機を移る場合）

**本番へデプロイ済み・DBの修正も適用済み。** 続きの作業は別の作業機からでも
そのまま再開できる（進捗はDBに入っている）。

### この日にやったこと
1. 詳細画面の「例文」と「訳」が別の文になっていた不具合を修正（2,452語を
   自動修復 + 176語を人手で訳し直し）。再発防止に `detail.example_ja_src`
   を記録し、表示側でも不一致なら訳を出さないようにした。
2. 詳細JSONに混入していた別言語の単語18件を修正。
3. ネイティブ速度をWPM実測（`scripts/tts_speed_test.py`）。現行nativeは
   learn比1.07倍でほぼ差が無いことを確認し、再生速度1.15倍を採用。
   英単語・フラッシュカードにも「ネイティブ」の選択肢を追加。
4. 未登録アクセス画面: 総数を種別ごとに先頭固定表示 + 明細を折りたたみ・
   種別フィルタ（既定は人間のみ）・列クリックで並べ替え。
5. 「バージョン情報」ページを新設（`release_notes.json`）。
6. メンテナンス予定をDB(`app_state`)に持たせ再起動なしで変更可能に。
   予告時刻の自動デプロイ `deploy/scheduled_deploy.sh` をVPSのcronに登録済み
   （5分おき・失敗時は自動ロールバック）。

詳細は [CHANGELOG.md](CHANGELOG.md) の ver1.2.10 を参照。

### 例文訳の人手点検 → 2026-08-22中に全件完了
残っていた6,729語（ID9400以上の477語＋ID4000〜8999の4,526語＋ID4000
未満の1,724語＋ID9000台の端数2語）をすべて確認・本番反映済み。
`list_example_ja_todo.py`の残りは**0**。経緯・手順は private リポジトリ
`business_plan`の`products/study_nyangailab/訳文点検_引き継ぎ.md`参照。

### 次回やること
1. **新規登録の必須項目変更（ver1.2.11・本番デプロイ済み）**の実機フィード
   バックを見る（氏名/フリガナ→任意アンケート化、呼んでほしいお名前を
   必須化）。
2. **domainタグ不一致の疑い**（fish sauce等3語が「ベトナム料理」なのに
   例文はタイ料理）を確認→照査→修正。1語が複数domainに属してよいこと、
   domainごとに意味が変わり得ることを踏まえ慎重に。詳細は
   `business_plan`の同ファイル末尾「派生で見つかった別問題」参照。
