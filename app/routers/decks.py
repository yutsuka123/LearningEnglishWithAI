...
作り、フラッシュ単語画面の`deck_id`絞り込みで学習する(2026-08-18〜。
以前はデッキ専用のクイズ/採点エンドポイントがあったが未使用のため撤去済み・
下部のコメント参照)。学習進捗(mastery)はデッキ別ではなく
`user_word_progress`にグローバルに保持し、詳細設定の基準で「覚えた」等を
判定する。
...
def _select_word_ids(conn, p: DeckCreate) -> list[int]:
    ...
    if p.word_ids:
        ids = list(dict.fromkeys(p.word_ids))
        ph = ",".join("?" * len(ids))
        cond = "" if include_banned else f"AND {banned_filter('words')}"
        # 実在チェックは include_banned の値によらず常に行う
        # (2026-09-14 DB分割対応: words↔deck_wordsのFOREIGN KEYがATTACH間
        # では効かなくなったため、以前はFK制約が黙って弾いてくれていた
        # 「存在しないword_id」がinclude_banned=True経路では素通りして
        # しまう穴があった。Fableレビュー指摘M3)。
        allowed = {
            r["id"] for r in conn.execute(
                f"SELECT id FROM words WHERE id IN ({ph}) {cond}",
                ids,
            ).fetchall()
        }
...
def deck_words_list(deck_id: int):
    ...
    with db() as conn:
        ...
        rows = conn.execute(
            ...
            f"WHERE dw.deck_id = ?{ban} ORDER BY w.english COLLATE NOCASE",
