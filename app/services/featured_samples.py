...
選定の方針: ①**未登録ゲストが無料で再生できる範囲(GUEST無料範囲)の語だけ**
(課金ロジックは変えない。範囲外の語を無料にはしない)、②音声が保存済みで
...
# 一覧の先頭に固定する厳選語(この順に並べる)。基礎英語とニッチ分野を
# 交互に混ぜ、先頭数語だけ見ても幅が伝わるようにしている。
FEATURED_WORDS: list[tuple[str, str]] = [
    ("sad", "基礎語彙"),                              # 基礎: 語源が意外
    ("roger", "アマチュア無線・無線通信"),             # 無線
    ("lasagna", "イタリア料理"),                       # 料理
    ("vampire", "生物(想像上)"),                       # 幻想生物(妖怪系)
    ("GPU", "半導体"),                                 # 専門用語
