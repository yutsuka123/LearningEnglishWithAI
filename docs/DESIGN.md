...
**音声の保存先（要検討・どちらか/併用）**:
- (A) ファイル: `data/audio/` に番号＋声で保存（実装が簡単・ストリーミング容易）。
- (B) DB保存: 例 `audio_assets(id, kind('word'|'phrase'|'word_ex'), ref_id,`
  `voice, mime, data BLOB, created_at, UNIQUE(kind,ref_id,voice))`。
  - 利点: 1ファイル(DB)で完結し同期/バックアップが容易、番号で一元管理。
  - 欠点: DBが肥大（数千音声で数百MB級）、配信時にメモリ展開。
  - 折衷: DBにはメタ(番号・声・パス/ハッシュ)だけ持ち、実体はファイル。
- 実装時に (A)/(B)/折衷 を決定。まずは番号でメタ管理し、実体はファイル＋
  キャッシュが無難。ユーザー要望によりDB(BLOB)保存も選べるよう抽象化する。
