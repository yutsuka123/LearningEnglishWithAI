# ruff: noqa: E501
"""2026-09-08に追加した地学(108語)・天文(116語)・生物学(107語)の学部専門
レベル語彙、計331語だけを対象に、単語見出し＋例文のTTS音声を生成する。

`scripts/build_audio.py`(既定)は`ORDER BY level ASC, id ASC`でDB全体の
未生成バックログ(約9,000語超)から順に処理するため、このバッチ(level
850〜990+)には到達しないと判明済み(`build_audio_math_advanced_
2026_09_08.py`と同じ課題)。同じ方式で3ドメインまとめて狙い撃ちする。

対象の特定方法: `add_geology_advanced.py`/`add_astronomy_advanced.py`/
`add_biology_advanced.py`のWORDS(english+domain組)をインポートして
idを正確に解決する(homographはdomainで一意化)。

Run:  python scripts/build_audio_geo_astro_bio_2026_09_08.py
"""
...
BATCH_SCRIPTS = [
    "add_geology_advanced.py",
    "add_astronomy_advanced.py",
    "add_biology_advanced.py",
]
