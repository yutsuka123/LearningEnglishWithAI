# ruff: noqa: E501
"""2026-09-08に追加した物理(124語)・化学(114語)の学部専門レベル語彙、
計238語だけを対象に、単語見出し＋例文のTTS音声を生成する。

`scripts/build_audio.py`(既定)は`ORDER BY level ASC, id ASC`でDB全体の
未生成バックログから順に処理するため、このバッチ(level 850〜990+中心、
一部300〜480の基礎語含む)には到達しないと判明済み(地学/天文/生物学
バッチ(`build_audio_geo_astro_bio_2026_09_08.py`)と同じ課題)。同じ方式で
2ドメインまとめて狙い撃ちする。

対象の特定方法: `add_physics_advanced.py`/`add_chemistry_advanced.py`の
WORDS(english+domain組)をインポートしてidを正確に解決する
(homographはdomainで一意化)。

Run:  python scripts/build_audio_physics_chem_2026_09_08.py
"""
...
BATCH_SCRIPTS = [
    "add_physics_advanced.py",
    "add_chemistry_advanced.py",
]
