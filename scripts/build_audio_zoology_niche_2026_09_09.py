# ruff: noqa: E501
"""2026-09-09に追加した動物学ニッチ6分野(両生類/甲殻類/軟体動物/
動物(魚類)/爬虫類/動物園・植物園、計295語)だけを対象に、単語見出し＋
例文のTTS音声を生成する。

`scripts/build_audio.py`(既定)は`ORDER BY level ASC, id ASC`でDB全体の
未生成バックログから順に処理するため、このバッチには到達しないと判明
済み(物理/化学バッチ`build_audio_physics_chem_2026_09_08.py`と同じ課題)。
同じ方式で狙い撃ちする。

対象の特定方法: `add_zoology_niche_2026_09_09.py`のWORDS(english+domain
組)をインポートしてidを正確に解決する(homographはdomainで一意化)。

Run:  python scripts/build_audio_zoology_niche_2026_09_09.py
"""
...
BATCH_SCRIPTS = [
    "add_zoology_niche_2026_09_09.py",
]
